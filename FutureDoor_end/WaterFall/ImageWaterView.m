...
{
    //每一列的视图初始化
    firstView = [[UIView alloc]initWithFrame:CGRectMake(0, SCROLLVIEWH, WIDTH, 0)];
    secondView = [[UIView alloc]initWithFrame:CGRectMake(WIDTH, SCROLLVIEWH, WIDTH, 0)];
    thridView = [[UIView alloc]initWithFrame:CGRectMake(WIDTH*2, SCROLLVIEWH, WIDTH, 0)];
    ...
    for (int i = 0; i<self.arrayImage.count; i++) {
        //0%3=0,0-2除3也不可能大于0
        if (i/rowNumber>0 && i%rowNumber==0) {
            row++;
        }
    ...
    scrollImageView = [[UIScrollImageView alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, SCROLLVIEWH) withPics:arrayImage];
